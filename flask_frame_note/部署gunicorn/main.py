...
def how_start1():
    ...
    start_f0 = time.time()
    # 如果gunicorn启动了2个(worker)进程(每个worker进程的线程数为3),当所有线程同时处理请求时,每个线程都会独立创建一个ProcessPoolExecutor(max_workers=5),那么最多会启动2*3*5=30个子进程并行该任务(计算斐波拉契数列),可能会导致并行效率下降(CPU超载)
...
def how_start3():
    ...
    #
    # # # gunicorn设置2(threads = 1 workers = 2):
    # # # 每个进程处理一个HTTP请求
